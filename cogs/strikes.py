...
from constants import (
    ...
    get_member_faction, get_leader_faction, is_staff
)
...
class StrikesCog(commands.Cog):
    ...
    async def strike(self, interaction: discord.Interaction,
        ...
        # ── Only leaders or staff can strike ──────────────────────────────────
        if not actor_leader_faction and not is_staff(actor):
            return await interaction.followup.send(
                "❌ Only faction leaders or staff may issue strikes.",
                ephemeral=True
            )
        ...
        # ── Leaders can only strike their own faction ─────────────────────────
        if actor_leader_faction and target_faction != actor_leader_faction:
            return await interaction.followup.send(
    ...
    async def viewstrike(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not actor_leader_faction and not is_staff(actor):
            return await interaction.followup.send(
                "❌ Only faction leaders or staff may view strikes.", ephemeral=True
            )

        target_faction = get_member_faction(user)

        # ── Leaders can only view their own faction ───────────────────────────
        if actor_leader_faction and target_faction != actor_leader_faction:
            return await interaction.followup.send(
    ...
    async def clearstrikes(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not actor_leader_faction and not is_staff(actor):
            return await interaction.followup.send(
                "❌ Only faction leaders or staff may clear strikes.", ephemeral=True
            )

        target_faction = get_member_faction(user)

        # ── Leaders can only clear their own faction ──────────────────────────
        if actor_leader_faction and target_faction != actor_leader_faction:
            return await interaction.followup.send(
                f"❌ {user.mention} is not in your faction.", ephemeral=True
            )
